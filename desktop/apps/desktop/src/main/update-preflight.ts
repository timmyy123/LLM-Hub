...
export function updateRestartSafetyError(safety: Exclude<UpdateRestartSafety, { state: "clear" }>): {
  ...
  if (safety.state === "blocked") {
    return {
      ...
      message: `LLM Hub Studio is still working on ${safety.activeRunCount} active task${safety.activeRunCount === 1 ? "" : "s"}.`,
    };
  }
  return {
    code: UPDATE_RESTART_UNKNOWN_ERROR_CODE,
    details: { activeRunCount: null },
    message: "LLM Hub Studio could not confirm whether tasks are still running.",
  };
}
